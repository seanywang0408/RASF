...
class get_model(nn.Module):
    def __init__(self, part_num=50, channel=3, num_points=2048):
    ...
    def forward(self, x, label):
        ...
        x = x.transpose(2, 1)
        if D > 3:
            points, feature = x[:,:,:3], x[:,:,3:]
        else:
            points = x
        x = torch.bmm(points, trans)
        if D > 3:
        ...
        out_max = out_max.view(-1, self.num_points)

        out_max = torch.cat([out_max,label.squeeze(1)],1)
        expand = out_max.view(-1, self.num_points+16, 1).repeat(1, 1, N)
